...
contract('Controller', accounts =>  {
  ...
  it("register schemes - check permissions for updating existing scheme", async () => {
     ...
     it("unregister self", async () => {
      ...
      it("addGlobalConstraint ", async () => {
       ...
       it("removeGlobalConstraint ", async () => {
         ...
         assert.equal(await controller.isGlobalConstraintRegistered(globalConstraints.address),true);
         assert.equal(await controller.isGlobalConstraintRegistered(globalConstraints1.address),true);
         assert.equal(await controller.isGlobalConstraintRegistered(globalConstraints2.address),false);
         assert.equal(await controller.isGlobalConstraintRegistered(globalConstraints3.address),true);
         assert.equal(await controller.isGlobalConstraintRegistered(globalConstraints4.address),true);
         ...
         assert.equal(await controller.isGlobalConstraintRegistered(globalConstraints4.address),false);
    ...
    it("externalTokenTransferFrom & ExternalTokenApproval", async () => {
        ...
        var standardToken = await ERC20Mock.new(avatar.address, 100);
        await avatar.transferOwnership(controller.address);
        tx = await controller.externalTokenApproval(standardToken.address,avatar.address,50);
    ...
    it("globalConstraints sendEther  add & remove", async () => {
     ...
     try {
      await controller.sendEther(web3.utils.toWei('1', "ether"),otherAvatar.address);
    ...
    it("globalConstraints externalTokenTransfer  add & remove", async () => {
     ...
     controller = await setup(accounts);
     var nativeReputation = await controller.nativeReputation();
     ...
     try {
      await controller.externalTokenApproval(standardToken.address,avatar.address,50);
