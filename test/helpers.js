...
export const setupOrganizationWithArrays = async function (daoFactory,daoFactoryOwner,founderToken,founderReputation,controller=0,cap=0) {
  var org = new Organization();
  var tx = await daoFactory.forgeOrg("testOrg","TEST","TST",daoFactoryOwner,founderToken,founderReputation,controller,cap,{gas: constants.ARC_GAS_LIMIT});
...
export const setupOrganization = async function (daoFactory,daoFactoryOwner,founderToken,founderReputation,controller=0,cap=0) {
  var org = new Organization();
  var tx = await daoFactory.forgeOrg("testOrg","TEST","TST",[daoFactoryOwner],[founderToken],[founderReputation],controller,cap,{gas: constants.ARC_GAS_LIMIT});
  assert.equal(tx.logs.length, 1);
  assert.equal(tx.logs[0].event, "NewOrg");
