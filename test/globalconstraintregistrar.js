...
const IntVoteMock = artifacts.require("./IntVoteMock.sol");

import { getValueFromLogs } from './helpers';

const setupOrg = async function() {
  await IntVoteMock.new();
  const intVoteMock = await IntVoteMock.deployed();
  const gcScheme = await GlobalConstraintRegistrar.deployed();
  const token = await DAOToken.at(await gcScheme.nativeToken);
  const fee = await DAOToken.at(await gcScheme.fee);

  const paramsHash = await gcScheme.setParameters(SOME_HASH, intVoteMock.address);
  opts = {initialSchemes: {
      addresses: [gcScheme.address],
      params: [paramsHash],
      permissions: ['0x00000007'],
      fees: [fee],
      feesTokenAddresses: [token],
  }};
  let org = await helpers.forgeOrganization(opts);

  await token.transfer(org.avatar.address, fee);
  await org.controller.externalTokenApprove(token, scheme, fee), {from: org.tyrantAddress};
  await gcScheme.registerOrganization(org.avatar);

  org.gcScheme = gcScheme;
  org.intVoteMock = intVoteMock;
