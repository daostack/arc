...
contract('ContributionReward', function(accounts) {
  ...
  //      });
   ...
   it("execute proposeContributionReward via genesisProtocol and redeem using Redeemer", async function() {
     ...
     var tx = await arcUtils.redeem(proposalId,testSetup.org.avatar.address,accounts[0]);

     const redeemerLog = await new Promise((resolve) => {
                 testSetup.contributionReward.RedeemReputation({_beneficiary: otherAvatar.address}, {fromBlock: tx.blockNumber})
                     .get((err,events) => {
                             resolve(events);
                     });
                 });
     console.log("redeemerLog",redeemerLog);
