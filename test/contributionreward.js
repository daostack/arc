...
var daoFactory, schemesFactory, actorsFactory;
const setupFactories = async function() {
  ...
  var contributionRewardLibrary = await ContributionReward.new({
    gas: constants.ARC_GAS_LIMIT
  });
  ...
  await schemesFactory.setContributionRewardLibraryAddress(
...
const checkRedeemedPeriods = async function(
  ...
  assert.equal(
    await testSetup.contributionReward.getRedeemedPeriods(proposalId, 0),
    ReputationPeriod
  );
  assert.equal(
    await testSetup.contributionReward.getRedeemedPeriods(proposalId, 1),
    nativeTokenPeriod
  );
  assert.equal(
    await testSetup.contributionReward.getRedeemedPeriods(proposalId, 2),
    EtherPeriod
  );
  assert.equal(
    await testSetup.contributionReward.getRedeemedPeriods(proposalId, 3),
    ExternalTokenPeriod
  );
};
...
const checkRedeemedPeriodsLeft = async function(
  ...
  assert.equal(
    await testSetup.contributionReward.getPeriodsToPay(
      proposalId,
      web3.utils.toBN(0)
    ),
    ReputationPeriod
...
const setup = async function(
  ...
  testSetup.orgNativeTokenFee = orgNativeTokenFee;
  ...
  if (genesisProtocol === true) {
    ...
    testSetup.contributionReward = await ContributionReward.at(
      (await schemesFactory.createContributionReward(
        testSetup.org.avatar.address,
...
contract("ContributionReward", accounts => {
  ...
  it("proposeContributionReward fees", async function() {
    ...
    var balanceBefore = await testSetup.standardTokenMock.balanceOf(
      testSetup.org.avatar.address
    );
    //give approval to scheme to do the fees transfer
    await testSetup.org.token.approve(
      testSetup.contributionReward.address,
      100
    );
    var tx = await testSetup.contributionReward.proposeContributionReward(
    ...
    assert.equal(tx.logs.length, 1);
    assert.equal(tx.logs[0].event, "NewContributionProposal");
    var balance = await testSetup.org.token.balanceOf(
      testSetup.org.avatar.address
    );
    assert.equal(balance.toNumber(), testSetup.orgNativeTokenFee);
    balance = await testSetup.standardTokenMock.balanceOf(
      testSetup.org.avatar.address
    );
    assert.equal(balance.toNumber(), balanceBefore.toNumber());
  });

  it("proposeContributionReward check owner vote", async () => {
  ...
  it("proposeContributionReward check beneficiary==0", async () => {
    ...
    assert.equal(
      await helpers.getValueFromLogs(tx, "_beneficiary"),
      accounts[0]
    );
  });
  ...
  it("execute proposeContributionReward  mint reputation ", async function() {
    ...
    tx = await testSetup.contributionReward.redeem(proposalId, [
      true,
      false,
      false,
      false
    ...
    assert.equal(
      rep.toNumber(),
      testSetup.reputationArray[1] + reputationReward
    );
  });
  ...
  it("execute proposeContributionReward  send ethers ", async function() {
    ...
    await helpers.increaseTime(periodLength + 1);
    await testSetup.contributionReward.redeem(proposalId, [
      false,
      false,
      true,
      false
  ...
  it("execute proposeContributionReward  send externalToken ", async function() {
    ...
    await testSetup.standardTokenMock.transfer(
      testSetup.org.avatar.address,
      30,
      { from: accounts[1] }
    );
    ...
    var tokens = await testSetup.standardTokenMock.balanceOf(
      otherAvatar.address
    );
  ...
  it("execute proposeContributionReward proposal decision=='no' send externalToken  ", async function() {
    ...
    try {
      ...
      assert(
        false,
        "redeem should revert because there was no positive voting"
      );
    } catch (ex) {
  ...
  it("execute proposeContributionReward  mint negative reputation ", async function() {
    ...
    assert.equal(
      rep.toNumber(),
      testSetup.reputationArray[0] + reputationReward
    );
  });
  ...
  it("execute proposeContributionReward via genesisProtocol and redeem using Redeemer", async function() {
    ...
    assert.equal(eth, ethReward);
    assert.equal(
      await testSetup.org.reputation.balanceOf(otherAvatar.address),
      reputationReward
    );
    assert.equal(
      await testSetup.org.token.balanceOf(otherAvatar.address),
      nativeTokenReward
    );
    ...
    var reputationGainAsProposer =
      (proposingRepRewardConstA * 1000 + proposingRepRewardConstB * 1000) /
      1000;
    assert.equal(
      reputation,
      1000 + reputationGainAsVoter + reputationGainAsProposer
    );
  });
  it("execute proposeContributionReward via genesisProtocol and redeem using Redeemer for negative proposal", async function() {
    ...
    //Vote with reputation to trigger execution
    var proposalId = await helpers.getValueFromLogs(tx, "_proposalId", 1);
    await testSetup.votingMachine.genesisProtocol.vote(proposalId, 2, 0, {
      from: accounts[0]
    });
