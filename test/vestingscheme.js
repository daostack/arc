...
contract("VestingScheme", accounts => {
  ...
  it("proposeVestingAgreement log", async () => {
    ...
    var tx = await testSetup.vestingScheme.proposeVestingAgreement(
      accounts[0],
      accounts[1],
      blockNumber,
      15,
  ...
  it("proposeVestingAgreement check assert _signaturesReqToCancel <= _signersArray.length", async function() {
    ...
    try {
      await testSetup.vestingScheme.proposeVestingAgreement(
        accounts[0],
        accounts[1],
        blockNumber,
        15,
      ...
      assert(
        false,
        "proposeVestingAgreement should fail - due to _signaturesReqToCancel > _signersArray.length !"
      );
    } catch (ex) {
  ...
  it("proposeVestingAgreement check assert _numOfAgreedPeriods > 0", async function() {
    ...
    try {
      ...
      assert(
        false,
        "proposeVestingAgreement should fail - due to _numOfAgreedPeriods > 0 !"
      );
    } catch (ex) {
      helpers.assertVMException(ex);
    }
  ...
  it("execute proposeVestingAgreement  -from none voting contact -should fail", async function() {
    ...
    try {
      ...
      assert(
        false,
        "execute should fail - due because it is not called from voting contract !"
      );
    } catch (ex) {
      helpers.assertVMException(ex);
    }
  ...
  it("execute proposeVestingAgreement controller -yes - check minting", async function() {
    ...
    assert.equal(
      await testSetup.org.token.balanceOf(testSetup.vestingScheme.address),
      amountPerPeriod * numberOfAgreedPeriods
    );
  });
  ...
  it("createVestedAgreement check agreement id ", async function() {
    ...
    var tx = await testSetup.vestingScheme.createVestedAgreement(
    ...
    tx = await testSetup.vestingScheme.createVestedAgreement(
      testSetup.standardTokenMock.address,
      accounts[0],
      accounts[1],
      blockNumber,
  ...
  it("createVestedAgreement check periodLength==0 ", async function() {
    ...
    try {
      await testSetup.vestingScheme.createVestedAgreement(
        testSetup.standardTokenMock.address,
        accounts[0],
        accounts[1],
        blockNumber,
      ...
      assert(
        false,
        "createVestedAgreement should fail - due to periodLength == 0"
      );
    } catch (ex) {
      helpers.assertVMException(ex);
    }
  ...
  it("createVestedAgreement check payment", async function() {
    ...
    assert.equal(
      await testSetup.standardTokenMock.balanceOf(
        testSetup.vestingScheme.address
      ),
      amountPerPeriod * numberOfAgreedPeriods
    );
  ...
  it("createVestedAgreement check assert _signaturesReqToCancel <= _signersArray.length", async function() {
    ...
    try {
      ...
      assert(
        false,
        "createVestedAgreement should fail - due to _signaturesReqToCancel > _signersArray.length !"
      );
    } catch (ex) {
      helpers.assertVMException(ex);
    }
  ...
  it("createVestedAgreement check _numOfAgreedPeriods > 0 ", async function() {
    ...
    try {
      ...
      assert(
        false,
        "createVestedAgreement should fail - due to _numOfAgreedPeriods > 0"
      );
    } catch (ex) {
      helpers.assertVMException(ex);
    }
  ...
  it("signToCancelAgreement from non signer", async function() {
    ...
    try {
      ...
      assert(
        false,
        "signToCancelAgreement should fail - due to accounts[1] is not authorized signer"
      );
    } catch (ex) {
      helpers.assertVMException(ex);
    }
  ...
  it("signToCancelAgreement wrong agreementId", async function() {
    ...
    try {
      ...
      assert(
        false,
        "signToCancelAgreement should fail - due to wrong agreementId"
      );
    } catch (ex) {
      helpers.assertVMException(ex);
    }
  ...
  it("signToCancelAgreement double sign attempt", async function() {
    ...
    try {
      ...
      assert(
        false,
        "signToCancelAgreement should fail - due to double sign attempt"
      );
    } catch (ex) {
      helpers.assertVMException(ex);
    }
  ...
  it("signToCancelAgreement check actual cancel", async function() {
    ...
    var balance = await testSetup.standardTokenMock.balanceOf(
      returnOnCancelAddress
    );
  ...
  it("revokeSignToCancelAgreement from non signer", async function() {
    ...
    try {
      ...
      assert(
        false,
        "revokeSignToCancelAgreement should fail - due to accounts[2] is not authorized signer"
      );
    } catch (ex) {
      helpers.assertVMException(ex);
    }
  ...
  it("revokeSignToCancelAgreement wrong agreementId", async function() {
    ...
    try {
      await testSetup.vestingScheme.revokeSignToCancelAgreement(
        agreementId + 1
      );
      assert(
        false,
        "revokeSignToCancelAgreement should fail - due to wrong agreement id"
      );
    } catch (ex) {
      helpers.assertVMException(ex);
    }
  ...
  it("revokeSignToCancelAgreement without signing before", async function() {
    ...
    try {
      ...
      assert(
        false,
        "revokeSignToCancelAgreement should fail - accounts[0] did not signed."
      );
    } catch (ex) {
      helpers.assertVMException(ex);
    }
  ...
  it("collect log periodsFromStartingBlock < agreement.numOfAgreedPeriods", async function() {
    ...
    var agreementId = await helpers.getValueFromLogs(tx, "_agreementId", 1);
    tx = await testSetup.vestingScheme.collect(agreementId, {
      from: beneficiary
    });
    assert.equal(tx.logs.length, 1);
    assert.equal(tx.logs[0].event, "Collect");
    var balance = await testSetup.standardTokenMock.balanceOf(beneficiary);
    var periodsFromStartingBlock = Math.floor(
      ((await web3.eth.getBlockNumber()) - startingBlock) / periodLength
    );
    assert.equal(
      balance.toNumber(),
      periodsFromStartingBlock * amountPerPeriod
    );
  });
  ...
  it("collect from none beneficiary", async function() {
    ...
    try {
      ...
      assert(
        false,
        "collect should fail - try to collect from none beneficiary "
      );
    } catch (ex) {
      helpers.assertVMException(ex);
    }
  ...
  it("collect update collectedPeriods and collect again", async function() {
    ...
    for (i = 0; i < numberOfAgreedPeriods; i++) {
      ...
      if (periodsFromStartingBlock < numberOfAgreedPeriods) {
        ...
        assert.equal(
          balance.toNumber(),
          numberOfAgreedPeriods * amountPerPeriod
        );
      }
    }
  });
