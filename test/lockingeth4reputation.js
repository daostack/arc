...
const setup = async function(
  ...
  if (_initialize === true) {
    await testSetup.lockingEth4Reputation.initialize(
...
contract("LockingEth4Reputation", accounts => {
  it("initialize", async () => {
    ...
    assert.equal(
      await testSetup.lockingEth4Reputation.maxLockingPeriod(),
      6000
    );
    assert.equal(
      await testSetup.lockingEth4Reputation.lockingEndTime(),
      testSetup.lockingEndTime
    );
  });
  ...
  it("lock", async () => {
    let testSetup = await setup(accounts);
    var tx = await testSetup.lockingEth4Reputation.lock(100, {
  ...
  it("cannot lock without initialize", async () => {
    ...
    try {
      await testSetup.lockingEth4Reputation.lock(100, {
        value: web3.utils.toWei("1", "ether")
      });
  ...
  it("lock with period == 0 should revert", async () => {
    let testSetup = await setup(accounts);
    try {
      await testSetup.lockingEth4Reputation.lock(0, {
        value: web3.utils.toWei("1", "ether")
      });
  ...
  it("lock over _maxLockingPeriod should revert", async () => {
    let testSetup = await setup(accounts);
    try {
      await testSetup.lockingEth4Reputation.lock(6001, {
        value: web3.utils.toWei("1", "ether")
      });
  ...
  it("redeem score ", async () => {
    ...
    tx = await testSetup.lockingEth4Reputation.lock(300, {
      from: accounts[1],
      value: web3.utils.toWei("1", "ether")
    });
  ...
  it("initialize is onlyOwner", async () => {
    ...
    try {
      await lockingEth4Reputation.initialize(accounts[1], 100, 0, 100, 6000, {
        from: accounts[1]
      });
