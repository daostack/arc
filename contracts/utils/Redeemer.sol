...
contract Redeemer {
    ...
    */
            ...
            uint256 crExternalTokenReward)
    {
        ...
        if (callContributionReward) {
            ...
            if (_contributionReward.getProposalExecutionTime(_proposalId) > 0) {
                ...
                contributionRewardRedeem(_contributionReward, _proposalId);
            }
        }
    }
    ...
    function contributionRewardRedeem(ContributionReward _contributionReward, bytes32 _proposalId)
    private
    returns (int256 reputation, uint256 nativeToken, uint256 eth, uint256 externalToken)
    {
        ...
        Avatar avatar = _contributionReward.avatar();
        address vault = address(avatar.vault());
        if ((ethReward == 0) || (vault.balance < ethReward)) {
        ...
        if ((externalTokenReward == 0) ||
            (IERC20(externalTokenAddress).balanceOf(address(avatar)) < externalTokenReward)) {
    ...
    returns (int256 reputation, uint256 nativeToken, uint256 eth, uint256 externalToken)
    {
        ...
        if ((ethReward == 0) || (address((_contributionRewardExt.avatar()).vault()).balance < ethReward)) {
            whatToRedeem[2] = false;
        } else {
            whatToRedeem[2] = true;
        }
