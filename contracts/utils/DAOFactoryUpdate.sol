...
contract DAOFactory is Initializable {
    ...
    */
    ...
    function forgeOrg (
        ...
        external
        returns(address) {
            ForgeOrgParams memory forgeOrgParams = abi.decode(_encodedForgeOrgParams, (ForgeOrgParams));
            uint64[3] memory packageVersion = getPackageVersion(forgeOrgParams.version);
            Avatar avatar =_forgeOrg(
                            forgeOrgParams.orgName,
                            forgeOrgParams.tokenInitData,
                            forgeOrgParams.founders,
                            forgeOrgParams.foundersTokenAmount,
                            forgeOrgParams.foundersReputationAmount,
                            packageVersion);
        ...
        private
    {
        ...
        for (uint256 i = 0; i < _schemesNames.length; i++) {
          //add avatar to encoded data and encode the call to initilize
            bytes schemeEncodedData = _schemesData.slice(startIndex, _schemesInitilizeDataLens[i]);
            (abi.encode(_avatar)).concat(_schemesData.slice(startIndex, _schemesInitilizeDataLens[i]));
