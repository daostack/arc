...
export class Organization {
  ...
  async initialize(opts) {
    ...
    this._schemeRegistrar = SchemeRegistrar.at(options.schemeRegistrar);
    // TODO: treat the next schems in the same way oas schemeRegistrar..
  ...
  async schemeRegistrar() {
    // returns the schemes can be used to register othe schemes
    // TODO: this is just a quick shortcut, what we really shoudl to is:
    // ls = this.schemes('SchemeRegistrar');
    // if ls is not of length 1, trhow an error
    // if it is of length 1, check the conditions for usage
    // if all is fine, return it
    const schemeRegistrar =  this._schemeRegistrar;
    ...
    const isControllerRegistered = await schemeRegistrar.isRegistered(avatar.address);
...
Organization.new = async function(opts) {
  ...
  const schemeRegistrar = await org._schemeRegistrar;
  ...
  // TODO: these are specific configuation options that should be settable in the options above
  const upgradeScheme = await org.upgradeScheme;
  ...
  // TODO: these are specific configuation options that should be settable in the options above
  const globalConstraintRegistrar = await org.globalConstraintRegistrar;
