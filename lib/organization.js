...
export class Organization {
  ...
  static async new(opts) {
    ...
    const defaults = {
        ...
        schemes: [
          {
            contract: SCHEME_TYPE_SCHEMEREGISTRAR,
            schemeAddress: settings.schemeRegistrar,
          },
          {
            contract: SCHEME_TYPE_UPGRADESCHEME,
            schemeAddress: settings.upgradeScheme,
          },
          {
            contract: SCHEME_TYPE_GLOBALCONTRAINTREGISTRAR,
    ...
    for (let i=0; i < schemes.length; i = i + 1) {
      if (schemes[i].contract === SCHEME_TYPE_SCHEMEREGISTRAR) {
      ...
      if (schemes[i].contract === SCHEME_TYPE_UPGRADESCHEME) {
      ...
      if (schemes[i].contract === SCHEME_TYPE_GLOBALCONTRAINTREGISTRAR) {
        // TODO: these are specific configuation options that should be settable in the options above
