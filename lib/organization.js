"use strict";
import { getValueFromLogs, NULL_ADDRESS, requireContract } from './utils.js';

const dopts = require('default-options');

const Avatar = requireContract("./Avatar.sol");
const Controller = requireContract("./Controller.sol");
const DAOToken = requireContract("./DAOToken.sol");
const GenesisScheme = requireContract("./GenesisScheme.sol");
import { GlobalConstraintRegistrar } from   '../lib/globalconstraintregistrar.js';
const Reputation = requireContract("./Reputation.sol");
const SimpleICO = requireContract("./SimpleICO.sol");
const SimpleVote = requireContract("./SimpleVote.sol");
import { SimpleContributionScheme } from   '../lib/simplecontributionscheme.js';
const TokenCapGC = requireContract("./TokenCapGC.sol");
const UpgradeScheme = requireContract("./UpgradeScheme.sol");
...
export class Organization {
  ...
  static async new(opts) {
    ...
    const defaults = {
        ...
        tokenSymbol: null,
        founders: [],
        votingMachine: settings.daostackContracts.SimpleVote.address,
        votePrec: 50,
  ...
  async proposeScheme(opts={}) {
    ...
    if (options.contract === 'SimpleICO') {
      ...
      const votingMachine = SimpleVote.at(options.params.boolVote);
      // check if voteApporveParams are known on the votingMachine
      await votingMachine.setParameters(this.reputation.address, options.params.votePrec);
      const voteApproveParams = await votingMachine.getParametersHash(this.reputation.address, options.params.votePrec);
  ...
  async proposeGlobalConstraint(opts= {}) {
    ...
    options.votingMachineHash = await SimpleVote.at(options.votingMachine).getParametersHash(options.reputation, options.absPrecReq);
  ...
  vote(proposalId, yesno, params) {
    ...
    return this.votingMachine.vote(proposalId, yesno, NULL_ADDRESS, params);
  }
