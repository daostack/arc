...
export class Organization {
  ...
  static async new(opts) {
    ...
    const defaults = {
        ...
        schemes: [
          {
            schemeType: SCHEME_TYPE_SCHEMEREGISTRAR,
            schemeAddress: settings.schemeRegistrar,
          },
          {
            schemeType: SCHEME_TYPE_UPGRADESCHEME,
            schemeAddress: settings.upgradeScheme,
          },
          {
            schemeType: SCHEME_TYPE_GLOBALCONTRAINTREGISTRAR,
    ...
    for (let i=0; i < schemes.length; i = i + 1) {
      if (schemes[i].schemeType === SCHEME_TYPE_SCHEMEREGISTRAR) {
      ...
      if (schemes[i].schemeType === SCHEME_TYPE_UPGRADESCHEME) {
      ...
      if (schemes[i].schemeType === SCHEME_TYPE_GLOBALCONTRAINTREGISTRAR) {
        // TODO: these are specific configuation options that should be settable in the options above
  ...
  async proposeScheme(opts={}) {
    ...
    if (options.schemeType === 'SimpleICO') {
      ...
      return proposalId;
    } else if (options.schemeType === 'SimpleContributionScheme') {
      // get the scheme
