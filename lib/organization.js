...
export class Organization {
  ...
  async schemeRegistrar() {
    ...
    const address = (await this.schemes(CONTRACT_SCHEMEREGISTRAR))[0].address;
    const scheme =  SchemeRegistrar.at(address);
  ...
  async upgradeScheme() {
    // returns the schemes can be used to register other schemes
    // TODO: error handling: throw an error if such a schem does not exist, and also if there is more htan one
    const address = (await this.schemes(CONTRACT_UPGRADESCHEME))[0].address;
    const scheme =  UpgradeScheme.at(address);
  ...
  async globalConstraintRegistrar() {
    ...
    // TODO: error handling: throw an error if such a schem does not exist, and also if there is more htan one
    const address = (await this.schemes(CONTRACT_GLOBALCONSTRAINTREGISTRAR))[0].address;
    const scheme =  GlobalConstraintRegistrar.at(address);
  ...
  async proposeScheme(opts={}) {
    ...
    if (options.contract === 'SimpleICO') {
      ...
      // get the scheme
      const scheme = await SimpleContributionScheme.at(options.address || settings.contributionScheme);
  ...
  async proposeGlobalConstraint(opts= {}) {
    ...
    if (options.contract==='TokenCapGC') {
      options.address = options.address || settings.tokenCapGC;
  ...
  vote(proposalId, yesno) {
    ...
    return this.votingMachine.vote(proposalId, yesno, NULL_ADDRESS);
  }
}
